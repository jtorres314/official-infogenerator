...
interface DattTemplateSettingsFormProps {
  currentTemplate: string;
  onSaveTemplate: (newTemplate: string) => void;
}

export const DattTemplateSettingsForm: React.FC<DattTemplateSettingsFormProps> = ({ currentTemplate, onSaveTemplate }) => {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      console.error("Error saving DATT template:", e);
    ...
    <div className="space-y-6">
      <div className="pb-5 border-b border-slate-700">
        <div className="flex items-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">{UI_TEXT.settings.dattTemplateFormTitle}</h2>
        </div>
        <p className="hidden sm:block mt-2 text-sm leading-6 text-slate-400">
          {UI_TEXT.settings.dattTemplateFormDescription} 
        </p>
      </div>
      ...
      <div className="mt-8 p-4 bg-slate-850 rounded-md border border-slate-700">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm text-slate-400">
          {Object.entries(placeholders)
            .filter(([key]) => ['eventDate', 'eventMunicipality', 'eventAddress', 'eventModality', 'officialRank', 'officialReport'].includes(key))
            .map(([key, desc]) => (
