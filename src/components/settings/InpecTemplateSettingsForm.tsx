...
interface InpecTemplateSettingsFormProps {
  currentTemplate: string;
  onSaveTemplate: (newTemplate: string) => void;
}

export const InpecTemplateSettingsForm: React.FC<InpecTemplateSettingsFormProps> = ({ currentTemplate, onSaveTemplate }) => {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      console.error("Error saving INPEC template:", e);
    ...
    <div className="space-y-6">
      <div className="pb-5 border-b border-slate-700">
        <div className="flex items-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">{UI_TEXT.settings.inpecTemplateFormTitle}</h2>
        </div>
        <p className="hidden sm:block mt-2 text-sm leading-6 text-slate-400">
          {UI_TEXT.settings.inpecTemplateFormDescription} 
        </p>
      </div>
      ...
      <div className="mt-8 p-4 bg-slate-850 rounded-md border border-slate-700">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm text-slate-400">
           {Object.entries(placeholders)
            .filter(([key]) => ['eventDate', 'eventMunicipality', 'officialRank', 'officialReport', 'penitentiaryName', 'eventModality'].includes(key))
            .map(([key, desc]) => (
