...
import { Save, ListChecks, AlertTriangle, UploadCloud, FileText as FileTextIcon, FileSliders } from 'lucide-react';

interface WebServiceTemplateSettingsFormProps { 
  currentTemplate: string;
  onSaveTemplate: (newTemplate: string) => void;
}

const webServicePlaceholderOrder: Array<keyof typeof UI_TEXT.settings.placeholders> = [
  'additionalAccused',
  'accusedName',
...
export const WebServiceTemplateSettingsForm: React.FC<WebServiceTemplateSettingsFormProps> = ({ currentTemplate, onSaveTemplate }) => { 
  ...
  const handleSave = () => {
    ...
    try {
      ...
      console.error("Error saving Web Service paragraph template:", e);
    ...
    <div className="space-y-6">
      <div className="pb-5 border-b border-slate-700">
        <div className="flex items-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">{UI_TEXT.settings.webServiceParagraphTemplateFormTitle}</h2>
        </div>
        <p className="hidden sm:block mt-2 text-sm leading-6 text-slate-400">
          {UI_TEXT.settings.webServiceParagraphTemplateFormDescription}
        </p>
      </div>
      ...
      <div className="p-4 bg-slate-850 rounded-lg shadow-md border border-slate-700 space-y-3">
        ...
        <TextArea
          id="webServiceParagraphTemplateEditor"
          label=""
          value={template}
          onChange={handleTemplateChange}
          rows={10} 
          className="font-mono text-sm bg-slate-700 border-slate-600 focus:ring-indigo-500 focus:border-indigo-500"
          placeholder="Escriba aquí su plantilla Handlebars para Web Service o cargue un archivo .txt..."
      ...
      <div className="p-4 bg-slate-850 rounded-lg shadow-md border border-slate-700 space-y-3">
        ...
        <FileUpload
          id="webServiceParagraphTemplateFile"
          label="" 
      ...
      <div className="mt-8 p-4 bg-slate-850 rounded-md border border-slate-700">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm text-slate-400">
          {webServicePlaceholderOrder.map(key => {
            const desc = placeholders[key as keyof typeof placeholders]; 
            if (desc) {
